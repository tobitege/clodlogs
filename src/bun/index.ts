...
import { generateUuidV7String } from "../shared/claude-rollout.ts";
...
const SETTINGS_FILE_NAME = "clodlogs-settings.json";
...
function resolveClaudeHomePath(claudeHome: string | null | undefined): string {
  return path.resolve(claudeHome ?? process.env.CLAUDE_HOME ?? DEFAULT_CLAUDE_HOME);
}
...
async function getEnvironmentCapabilities(
  ...
  const resolvedClaudeHome = resolveClaudeHomePath(claudeHome);
  const claudeHomeReadable = await canReadDirectory(resolvedClaudeHome);
  const claudeHomeWritable = await canWriteDirectoryOrParent(resolvedClaudeHome);
  ...
  if (!claudeHomeReadable) {
    notes.push(
      "Claude home is missing or not readable. Session browsing will stay empty until this path becomes available.",
    );
  }
  if (!claudeHomeWritable) {
    notes.push("Claude home is not writable. Temporary sanitized copies may still be available.");
  }
  if (!gitAvailable) {
  ...
  const overallKind: EnvironmentCapabilities["overallKind"] = !claudeHomeReadable
    ? "error"
    : !claudeHomeWritable || !gitAvailable || !ripgrepAvailable
      ? "warning"
      : "success";
  ...
  const summary =
    overallKind === "success"
      ...
      : overallKind === "error"
        ? "Claude home is not ready for normal session browsing."
        : "Some runtime capabilities are limited, but clodlogs can still run.";
...
async function appendSessionIndexThreadName(options: {
  ...
  const indexPath = path.join(options.claudeHome, "session_index.jsonl");
...
async function renameSessionThreadName(options: {
  ...
  throw new Error("Renaming Claude project-log sessions is not supported.");
}
...
function buildSanitizedSessionMetaRecord(options: {
  ...
  return {
    ...
    payload: {
      ...
      source: "clodlogs_sanitized_copy",
      clodlogs_sanitized_copy: {
...
async function createSanitizedSessionOutputDirectory(
  ...
  const safeStem = path
    ...
    .replace(/^-|-$/g, "")
    .slice(0, 80);
  const root = path.join(os.tmpdir(), "clodlogs", "sanitized-sessions");
...
async function createSanitizedSessionCopy(
  ...
  const resolvedClaudeHome = resolveClaudeHomePath(options.claudeHome);
  const readdedSessionPath = null;

  if (options.reAddToCurrentDay) {
    throw new Error("Re-adding sessions is not supported for Claude project-log storage.");
  }
  ...
  if (
    ...
    options.onProgress(90, "writing", "Writing canonical Claude session copy...");
  ...
  options.onProgress(
    ...
    writtenReaddedSessionPath
      ? "Sanitized session copy is ready and was re-added to today's Claude sessions."
...
function startSanitizedCopyJob(options: {
  ...
  void (async () => {
    try {
      ...
      Utils.showNotification({
        title: "clodlogs sanitized output ready",
      ...
      if (wasExportCancelled(error)) {
        ...
        Utils.showNotification({
          title: "clodlogs sanitization failed",
          body: path.basename(options.sessionFilePath),
        });
      }
    } finally {
...
const rpc = BrowserView.defineRPC<ClodlogsRPC>({
  ...
  handlers: {
    requests: {
      ...
      getEnvironmentCapabilities: async ({ claudeHome }) =>
        getEnvironmentCapabilities(claudeHome),
      renameSessionThreadName: async ({ claudeHome, threadId, threadName }) =>
...
mainWindow.webview.on("dom-ready", () => {
  console.log("clodlogs mainview ready");
});

console.log(`clodlogs started with ${url}`);
