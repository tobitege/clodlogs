...
export type FindClaudeSessionsOptions = {
...
export type FindClaudeSessionsResult = {
  claudeHome: string;
...
type ClaudeSessionFolderDateRange = {
  active: boolean;
  fromKey: number | null;
  toKey: number | null;
};
...
async function probeSessionFile(filePath: string): Promise<SessionFileProbe | null> {
  try {
    ...
    cwd ??= decodeClaudeProjectFolderPath(path.basename(path.dirname(filePath))) ?? path.dirname(filePath);
    updatedAt ??= startedAt ?? stat.mtime.toISOString();
...
function decodeClaudeProjectFolderPath(folderName: string): string | null {
  const match = /^([a-z])--(.+)$/i.exec(folderName);
  if (!match) {
    return null;
  }
...
function buildClaudeSessionFolderDateRange(
  dateFrom: string | null | undefined,
  dateTo: string | null | undefined,
): ClaudeSessionFolderDateRange {
...
function claudeSessionMetaMatchesDateRange(
  meta: NonNullable<SessionFileProbe["meta"]>,
  range: ClaudeSessionFolderDateRange,
): boolean {
  if (!range.active) {
    return true;
  }

  const sessionDateKey = parseTimestampDateKey(meta.updatedAt ?? meta.startedAt);
  if (sessionDateKey === null) {
    return false;
  }

  return (
    (range.fromKey === null || sessionDateKey >= range.fromKey) &&
    (range.toKey === null || sessionDateKey <= range.toKey)
  );
}

function parseTimestampDateKey(value: string | null | undefined): number | null {
...
export async function findClaudeSessions(
  options: FindClaudeSessionsOptions = {},
): Promise<FindClaudeSessionsResult> {
  const normalizedTargetDirectory = normalizeOptionalPathInput(options.targetDirectory);
  const normalizedClaudeHome = normalizeOptionalPathInput(options.claudeHome);
  ...
  const claudeHome = resolveFilesystemPath(
    normalizedClaudeHome ?? process.env.CLAUDE_HOME ?? DEFAULT_CLAUDE_HOME,
  );
  ...
  const dateRange = buildClaudeSessionFolderDateRange(options.dateFrom, options.dateTo);
  ...
  const sessionFiles = (await collectJsonlFiles(projectsDirectory)).map((file) => ({
    file,
    kind: "live" as const,
  }));
  const candidateSearchPaths = [projectsDirectory];
  ...
  for (const entry of sessionFiles) {
    ...
    if (!claudeSessionMetaMatchesDateRange(meta, dateRange)) {
      continue;
    }
...
export async function getSessionDetailMetrics(
  ...
  let toolCallCount = 0;
  const tokenUsageByMessage = new Map<string, SessionTokenUsage>();
  ...
  for await (const event of streamJsonlRecords(sessionFilePath, {
    ...
    largestParsedLineBytes = Math.max(largestParsedLineBytes, event.byteLength);
    const record = event.record;
    if (isHumanClaudeUserMessage(record)) {
      userMessageCount += 1;
    }
    toolCallCount += countClaudeToolUses(record);
    addClaudeUsageFromRecord(record, tokenUsageByMessage);
  }

  return {
    interactionCount: userMessageCount,
    toolCallCount,
    tokenUsage: sumClaudeTokenUsage(tokenUsageByMessage),
    fileSizeBytes,
...
export async function readSessionTokenUsage(
  ...
  for await (const event of streamJsonlRecords(sessionFilePath, {
    ...
    addClaudeUsageFromRecord(event.record, tokenUsageByMessage);
  ...
  return {
    ...
    fileSizeBytes,
    oversizedLineCount,
    tokenCountRows: tokenUsageByMessage.size,
  };
}

function isHumanClaudeUserMessage(record: JsonlRecord): boolean {
  if (record.type !== "user") {
    return false;
  }
  ...
  return Boolean(text) && !looksLikeBootstrapContext(text);
}

function countClaudeToolUses(record: JsonlRecord): number {
  const message = asObject(record.message);
  if (!message || !Array.isArray(message.content)) {
    return 0;
  }

  return message.content.filter((item) => asObject(item)?.type === "tool_use").length;
}

function addClaudeUsageFromRecord(
  record: JsonlRecord,
  target: Map<string, SessionTokenUsage>,
): void {
  ...
  target.set(usage.key, usage.tokenUsage);
}

function extractClaudeUsage(
  record: JsonlRecord,
): { key: string; tokenUsage: SessionTokenUsage } | null {
  const message = asObject(record.message);
  ...
  const inputTokens = asNonNegativeInteger(usage.input_tokens) ?? 0;
  const cacheCreationInputTokens =
    asNonNegativeInteger(usage.cache_creation_input_tokens) ?? 0;
  const cacheReadInputTokens = asNonNegativeInteger(usage.cache_read_input_tokens) ?? 0;
  const outputTokens = asNonNegativeInteger(usage.output_tokens) ?? 0;
  if (
    inputTokens === 0 &&
...
function sumClaudeTokenUsage(
  usageByMessage: Map<string, SessionTokenUsage>,
): SessionTokenUsage | null {
  if (usageByMessage.size === 0) {
    return null;
  }
...
export async function readSessionTranscript(
  ...
  for await (const event of streamJsonlRecords(sessionFilePath, {
    ...
    const nextEntries = buildClaudeTranscriptEntries(record, entries.length, {
...
function buildClaudeTranscriptEntries(
  record: JsonlRecord,
  nextIndex: number,
  ...
  if (!message || !role || !Array.isArray(message.content)) {
    return [];
  }

  const timestamp = typeof record.timestamp === "string" ? record.timestamp : null;
  const entries: SessionTranscriptEntry[] = [];
  const pushEntry = (entry: Omit<SessionTranscriptEntry, "index">): void => {
    entries.push({
  ...
  for (const rawItem of message.content) {
    ...
    if (type === "tool_result") {
      ...
      pushEntry({
        ...
        role: null,
        timestamp,
        title: `Tool Output${toolUseId}`,
        text: prettyStructuredText(item.content),
        language: "text",
      });
    }
  }
...
function shouldIncludeTranscriptEntry(
  entry: SessionTranscriptEntry,
  includeToolCallResults: boolean,
): boolean {
  if (entry.kind === "tool_call" || entry.kind === "tool_output") {
    return includeToolCallResults;
  }

  return true;
}

function renderMarkdownTranscriptEntry(entry: SessionTranscriptEntry): string {
  const timestamp = entry.timestamp ?? "unknown-time";
  if (entry.kind === "message") {
    return `### ${timestamp} ${entry.title}\n\n${entry.text}`;
  }

  const language = entry.language === "json" ? "json" : "text";
  return [
...
function renderHtmlTranscriptEntry(entry: SessionTranscriptEntry): string {
  const roleClass =
    entry.kind === "message" && entry.role
      ? `bubble-${escapeHtml(entry.role)}`
      : entry.kind === "reasoning"
        ? "bubble-reasoning"
        : "bubble-tool";
  const body =
    entry.language === "markdown" && entry.kind === "message"
      ? entry.text
          .split("\n\n")
  ...
  return `
    <div class="bubble ${roleClass}">
      <div class="bubble-header">
        ...
        <span>${escapeHtml(entry.timestamp ?? "unknown-time")}</span>
      </div>
...
async function exportSessionJsonlToMarkdownStreamed(
  ...
  try {
    ...
    for await (const event of streamJsonlRecords(sessionFilePath, {
      ...
      const record = event.record;
      const entries = buildClaudeTranscriptEntries(record, transcriptEntryCount, {
        sawFirstUserMessage,
        setSawFirstUserMessage: () => {
          sawFirstUserMessage = true;
        },
      ...
      for (const entry of entries) {
        if (!shouldIncludeTranscriptEntry(entry, normalizedOptions.includeToolCallResults)) {
          continue;
        }
        if (transcriptEntryCount > 0) {
          await writeTextToStream(stream, "\n\n");
        }
        await writeTextToStream(stream, renderMarkdownTranscriptEntry(entry));
  ...
  try {
    ...
    for await (const event of streamJsonlRecords(sessionFilePath, {
      ...
      for (const entry of entries) {
        ...
        await writeTextToStream(stream, `${renderHtmlTranscriptEntry(entry)}\n`);
        transcriptEntryCount += 1;
      }
...
async function readSessionExportMetadata(
  ...
  const probe = await probeSessionFile(sessionFilePath);
  if (!probe?.meta) {
    throw new Error(`No Claude session metadata found in ${sessionFilePath}`);
  }
  ...
  for await (const event of streamJsonlRecords(sessionFilePath, {
    ...
    if (typeof event.record.version === "string") {
      cliVersion ??= event.record.version;
    }
    const message = asObject(event.record.message);
    if (message && typeof message.model === "string") {
      modelProvider ??= message.model;
    }
    if (cliVersion && modelProvider) {
      break;
    }
  }
  ...
  return {
    id: probe.meta.id,
    startedAt: probe.meta.startedAt,
    cwd: probe.meta.cwd,
    originator: "Claude Code",
    cliVersion,
    source: probe.meta.source,
    modelProvider,
  };
}
