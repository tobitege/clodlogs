...
export function formatClaudeRolloutTimestamp(date: Date): string {
  return [
    date.getFullYear(),
...
export function buildClaudeCurrentDaySessionDirectory(
  claudeHome: string,
  date: Date = new Date(),
): string {
  return path.join(
...
export function buildClaudeRolloutFileName(
  threadId: string,
  date: Date = new Date(),
): string {
  return `rollout-${formatClaudeRolloutTimestamp(date)}-${threadId}.jsonl`;
}

export function buildClaudeCurrentDayRolloutPath(
  claudeHome: string,
  threadId: string,
  date: Date = new Date(),
): string {
  return path.join(
    buildClaudeCurrentDaySessionDirectory(claudeHome, date),
    buildClaudeRolloutFileName(threadId, date),
  );
}
