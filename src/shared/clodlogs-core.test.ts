...
} from "./clodlogs-core.ts";
import {
  buildClaudeCurrentDayRolloutPath,
  buildClaudeRolloutFileName,
  formatClaudeRolloutTimestamp,
  generateUuidV7String,
} from "./claude-rollout.ts";
import {
...
function encodeClaudeProjectFolder(cwd: string): string {
  return cwd
...
async function writeClaudeProjectSessionFile(options: {
  claudeHome: string;
  id: string;
  cwd: string;
  timestamp: string;
  ...
  const sessionDirectory = path.join(
    options.claudeHome,
    "projects",
    encodeClaudeProjectFolder(options.cwd),
  );
...
describe("claude rollout naming helpers", () => {
  ...
  test("builds canonical current-day rollout paths", () => {
    ...
    expect(buildClaudeRolloutFileName(threadId, date)).toBe(
      "rollout-2026-03-17T09-08-07-0195a7d2-93f1-7abc-8def-0123456789ab.jsonl",
    );
    expect(buildClaudeCurrentDayRolloutPath("C:\\Users\\[username]\\.claude", threadId, date)).toBe(
      path.join(
...
describe("session discovery filters", () => {
  test("filters sessions by Claude metadata timestamps from project files", async () => {
    const tempDir = await createTempDir("clodlogs-date-filter-");
    ...
    const result = await findClaudeSessions({
      claudeHome,
    ...
    expect(result.sessions.map((session) => session.id).sort()).toEqual(["metadata-date-match"]);
    expect(result.sessions[0]?.threadName).toBe("Matching Claude session");
  });
});

describe("large-session hardening", () => {
  test("skips automatic detail analysis for oversized files", async () => {
    const tempDir = await createTempDir("clodlogs-large-session-");
  ...
  test("returns Claude token usage from assistant message usage", async () => {
    const tempDir = await createTempDir("clodlogs-token-usage-");
  ...
  test("scans token usage without parsing unrelated transcript rows", async () => {
    const tempDir = await createTempDir("clodlogs-token-scan-");
    ...
    const usageRow = JSON.stringify({
      type: "assistant",
      sessionId: "token-scan",
      cwd: tempDir,
      timestamp: "2026-03-14T12:01:00.000Z",
      requestId: "req-token-scan",
      message: {
        id: "msg-token-scan",
        role: "assistant",
        content: [{ type: "text", text: "hi" }],
  ...
  test("returns partial detail metrics when a row exceeds the bounded line limit", async () => {
    const tempDir = await createTempDir("clodlogs-oversized-row-");
    const sessionPath = path.join(tempDir, "oversized-row.jsonl");
    const fileHandle = await fs.open(sessionPath, "w");

    const userMessage = JSON.stringify({
      type: "user",
      sessionId: "oversized-row",
      cwd: tempDir,
      timestamp: "2026-03-14T12:00:00.000Z",
      message: {
        role: "user",
        content: [{ type: "text", text: "hello from a normal row" }],
      },
    });
  ...
  test("streams markdown export for a normal session transcript", async () => {
    const tempDir = await createTempDir("clodlogs-export-");
    const sessionPath = path.join(tempDir, "normal-session.jsonl");

    const userMessage = JSON.stringify({
      type: "user",
      sessionId: "session-test-id",
      cwd: "D:\\github\\clodlogs",
      timestamp: "2026-03-14T12:01:00.000Z",
      message: {
        role: "user",
    ...
    expect(reconstructed.responseItems).toEqual([
      ...
      },
      {
        ...
        action: {
          ...
          command: ["rg skipped src"],
          timeout_ms: null,
          working_directory: "d:\\github\\clodlogs",
          env: null,
          user: null,
        },
      },
