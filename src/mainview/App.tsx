...
import type {
  FindClaudeSessionsResult,
  SessionDetailMetrics,
  SessionMetaMatch,
  SessionTokenUsage,
  SessionTranscriptEntry,
  SessionTranscriptResult,
} from "../shared/clodlogs-core.ts";
import type {
  ClodlogsRPC,
...
const REPOSITORY_URL = "https://github.com/tobitege/clodlogs";
const SESSION_BROWSER_MAX_ENTRIES = 10000;

const rpc = Electroview.defineRPC<ClodlogsRPC>({
...
function describeScope(
  result: FindClaudeSessionsResult | null,
  browseMode: BrowseMode,
): string {
  if (!result) {
    return "Loading";
...
function getExportDestinationHint(
  ...
  if (format === "markdown") {
    return "Choose a folder and clodlogs will write the Markdown file into it.";
  }

  return htmlExportUsesFilePicker(includeImages, inlineImages)
    ? "Save a single self-contained HTML file."
    : "Choose a folder and clodlogs will write the HTML file plus a sibling .assets folder.";
}
...
function App() {
  const [result, setResult] = useState<FindClaudeSessionsResult | null>(null);
  ...
  const claudeHomeRef = useRef(claudeHome);
  ...
  useEffect(() => {
    claudeHomeRef.current = claudeHome;
  }, [claudeHome]);
  ...
  useEffect(() => {
    ...
    environmentCapabilitiesState.capabilities?.claudeHome,
  ]);
  ...
  async function loadSessions(
    ...
    try {
      const nextResult = await getRpc().request.loadSessions({
        claudeHome: claudeHome.trim() || null,
        targetDirectory,
        cwdOnly: nextCwdOnly,
      ...
      if (!claudeHomeRef.current.trim()) {
        setClaudeHome(nextResult.claudeHome);
      }
      ...
      void refreshEnvironmentCapabilities(nextResult.claudeHome);
    } catch (loadError) {
      if (requestId !== loadRequestIdRef.current) {
        return;
      }
      ...
      void refreshEnvironmentCapabilities(claudeHome.trim() || null);
    } finally {
      if (requestId === loadRequestIdRef.current) {
        setLoading(false);
      }
  ...
  const canReAddToCurrentDay = false;
  const canRenameSessions = false;
  const canSanitizeSessions = false;
  const isClodlogsSanitizedSession = activeSession?.source === "clodlogs_sanitized_copy";
  const renameSessionDisabledReason =
    environmentCapabilitiesState.kind === "error"
      ...
      : environmentCapabilitiesState.kind === "loading"
        ? "Checking whether session titles can be changed..."
        : canRenameSessions
          ? null
          : "Claude project-log sessions cannot be renamed from clodlogs.";
  const reAddToCurrentDayDisabledReason =
    environmentCapabilitiesState.kind === "error"
      ? environmentCapabilitiesState.errorMessage ?? "Environment capability check failed."
      : environmentCapabilitiesState.kind === "loading"
        ? "Checking whether clodlogs can re-add sessions..."
        : canReAddToCurrentDay
          ? null
          : "Claude project logs do not use a current-day sessions folder.";
        ...
        <header className="hero">
          <div className="hero-content">
            ...
            <p className="hero-copy">
              <span>Browse and export your Claude sessions with ease.</span>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                aria-label="Open the clodlogs GitHub repository"
              ...
              <div className="environment-status-info">
                ...
                <span className="environment-summary-text">
                  {environmentCapabilitiesState.kind === "error"
                    ...
                    : environmentCapabilitiesState.kind === "loading"
                      ? "Checking Claude home access and helper tools..."
            ...
            {environmentExpanded && (
              <>
                <p className="environment-path">
                  Claude home: {environmentCapabilities?.claudeHome ?? (claudeHome || "Detecting...")}
                </p>
                {environmentCapabilities && (
                  <div className="environment-capability-list">
                    <EnvironmentCapabilityLine
                      available={environmentCapabilities.claudeHomeReadable}
                      label="Claude home read"
                    />
                    <EnvironmentCapabilityLine
                      available={environmentCapabilities.claudeHomeWritable}
                      label="Claude home write"
                    />
                    <EnvironmentCapabilityLine
              ...
              <div className="detail-content">
                <div className="detail-header">
                  ...
                  <div className="filter-actions detail-actions">
                    ...
                    {canSanitizeSessions && !isClodlogsSanitizedSession && (
                      <button
    ...
    props.metricsState.kind === "loading"
      ...
      : props.metricsState.kind === "error"
        ...
        : props.metricsState.metrics?.analysisKind === "skipped"
          ...
          : "No Claude usage data was found in this session.";
      ...
      >
        ...
        {props.state.kind === "idle" && (
          <div className="dialog-warning">
            ...
            <p>
              clodlogs will scan the current filtered set and sum Claude usage totals. You can cancel
      ...
      >
        <div className="export-dialog-header">
          <div>
            ...
            <p>Claude project-log session titles cannot be edited from clodlogs.</p>
          </div>
          <button
      ...
      >
        <div className="export-dialog-header">
          <div>
            ...
            <p>Choose where clodlogs should write the derived sanitized session outputs.</p>
          </div>
          <button
        ...
        <div className="export-dialog-options">
          ...
          >
            ...
            <span className="dialog-option-copy">
              ...
              <small>
                Create an additional live session copy. Claude project-log storage does not
                currently support this action.
              </small>
        ...
        <div className="dialog-destination">
          ...
          <strong>
            {props.createJsonlCopy && props.reAddToCurrentDay
              ? "Temporary output folder plus live session copy"
          ...
          <p>
            {props.reAddToCurrentDay
              ? "clodlogs leaves the source session unchanged and keeps the report in a temp folder."
              : "clodlogs writes all sanitization artifacts into a fresh temp subfolder and leaves the source session unchanged."}
          </p>
        </div>
